...
| Service | Role | Notes |
|---|---|---|
| Claude Code | Primary dev agent | Connected to GitHub repos |
| Vercel | Hosting + deployment | Auto-deploys on push to main |
| Alchemy API | Onchain data | Free tier — flag if limits approach |
| Namecheap | Domain management | — |
| GitHub Issues + Kanban | Project management | Backlog → In Progress → In Review → Done |

> ⚠️ Always check Vercel deployment status after every push to main.
...
- **Web3:** ethers.js (current), open to Wagmi/Viem for new projects
...
| Layer | Product | Role |
|---|---|---|
| 1 | WalletRep | Private reputation score |
| 2 | wlltresume | Public onchain resume |
| 3 | Wallet Passport | Portable soulbound credential NFT (Base) |
...
### Workflow
1. I describe goals and context in Claude chat
2. Claude chat generates a structured prompt for Claude Code
3. Claude Code executes — one prompt per feature chunk, never bundle too much
4. Test locally before moving to next prompt
5. Push to main → Vercel auto-deploys → verify deployment
...
- Remind proactively if something risks exposing a secret (e.g. before a push)
...
Always present options with tradeoffs — never make the decision silently:
```
...
### Communication Style
- No traditional dev background — explain technical decisions in plain language
...
|---|---|---|
| 1 | wlltresume — mobile responsive (Issue #8) | Next up |
| 2 | wlltresume × WalletRep — cross-reference integration | Done ✅ |
| 3 | Wallet Passport — soulbound NFT credential (Base) | PRD done |
| 4 | Personal portfolio repo | Ongoing |
| 5 | Medium blog cadence | Ongoing |
...
- Never delete `.gitignore` — if you do, recover it from git history and notify Bojan

---

## 11. Changelog

When a feature is moved to Done, update `CHANGELOG.md` under `[Unreleased]`.

When a meaningful set of changes is deployed, bump the version using semantic versioning:
...
- **Next.js audit warnings:** 4 high severity npm audit warnings on Next.js 14 — assessed as low risk for current usage pattern. Do **not** run `npm audit fix --force` as it would upgrade to Next.js 16 (breaking change). Revisit during a planned upgrade.
- **wlltrep `.gitignore`:** Was accidentally deleted once, restored from git history. Always add files explicitly — never `git add .` in this repo.
